...
declare function trigger:do-index( $value as xs:string?, $path as xs:anyURI) {
    let $index :=
<doc>
            <field name="" store="yes">{$value}</field>
        </doc>    
    let $null := ft:index($path, $index, false())
    return
        ()
    ...
    typeswitch ($root)
        ...
        default return
            if ($root/@class eq 'page') then                
                ...
                return
                    (: GM: use blank value for research
                    ( trigger:do-index("page", concat("[[", $page, "]]", string-join($root//xhtml:p/string(), " ")), $path), $page )
                    :)
                    ( trigger:do-index(concat("[[", $page, "]]", string-join($root//xhtml:p/string(), " ")), $path), $page )
            else
                $page
};
    ...
    if (util:is-binary-doc($uri)) then
        ...
        return
            if (ends-with($uri, ".pdf")) then
                let $log := util:log("info", concat("indexing ",$uri))    
